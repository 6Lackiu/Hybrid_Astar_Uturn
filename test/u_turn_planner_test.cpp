...
int main(int argc, char const *argv[]) {
    ...
    liu_algo::MotionConfig motion_config = {
    ...
    liu_algo::hybrid_a_star::SearchConfig search_config = {
    ...
    liu_algo::Lane start_lane;
    liu_algo::Lane end_lane;
    ...
    if (argc > 7 && argv[7] != NULL) {
        ...
        end_lane.center_line = {{std::stod(lane_data[0]), std::stod(lane_data[1]), M_PI * std::stod(lane_data[2])}};
        lane_stream.close();
    }
    else {
    ...
    std::ofstream out_stream(lane_data_file_name.empty() ? "trajectory.txt" : lane_data_file_name + ".traj", std::ofstream::trunc);
    double trajectory_length = 0.0;
    auto first_point = trajectory.front();
    for (const auto &point : trajectory) {
        out_stream << point.x << "," << point.y << "," << point.theta << "," << point.kappa << std::endl;
        trajectory_length += liu_algo::utils::Distance(point.x, point.y, first_point.x, first_point.y);
        first_point = point;
    }
    out_stream.close();
    ...
    double R = liu_algo::utils::Distance(start_lane.center_line.back().x, start_lane.center_line.back().y, goal.x, goal.y) / 2;
    std::cout << "TEST:"
