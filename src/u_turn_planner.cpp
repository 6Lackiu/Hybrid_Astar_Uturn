...
void UTurnPlanner::SetEnvironment(const Lane &start_lane, const Lane &end_lane) {
    ...
    start_point_ = start_lane.center_line.back();       // return last one
    end_lane_ = end_lane;
    end_point_ = end_lane.center_line.front();          // return first one
...
std::vector<Point> UTurnPlanner::Plan(std::vector<liu_algo::Point> *a_star_output, bool* goal_reached) {
    ...
    *a_star_output = a_star_traj;
    // fit X and Y
    PolynomialCoefs coefs_x = utils::PolynomialFit(a_star_traj_x_t, 5);
    PolynomialCoefs coefs_y = utils::PolynomialFit(a_star_traj_y_t, 5);
    ...
    for (unsigned int i = 0; i < a_star_traj.size(); i++) {
        ...
        smoothed_trajectory[i].theta = utils::ComputeTheta(coefs_x_d, coefs_y_d,
                                                           i * search_config.time_step);
        smoothed_trajectory[i].kappa = utils::ComputeKappa(coefs_x_d, coefs_x_dd,
                                                           coefs_y_d, coefs_y_dd,
                                                           i * search_config.time_step);
    }
