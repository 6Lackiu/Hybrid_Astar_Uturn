...
int HybridAStarSolver::DiscretizeTheta(double continuous_theta) {
    /*
  Takes an angle (in radians) and returns which "stack" in the 3D configuration space this angle corresponds to.
...
std::vector<State> HybridAStarSolver::Expand(const ExploreMap &explored, const State &current, const GridPoint &discrete_goal) {
    double x = current.x;
    double y = current.y;
    double theta = current.theta;

    std::vector<State> next_states;     // State: x y theta actual_cost estimated_cost
...
std::vector<State> HybridAStarSolver::GetSteps(const StateMap &parent, const Point &start, State final) {
...
std::vector<State> HybridAStarSolver::Solve(const Point &start, const Point &goal, bool *goal_reached) {
    ...
    // frontier
    StateHeap frontier;     // pop the lowest estimated_cost State first
    State init;
    init.x = start.x;
    init.y = start.y;
    ...
    explored[init_discrete_point.theta][init_discrete_point] = ExploreState::ON_FRONTIER;       // 用init_discrete_point.theta确定vector位置
    ...
    while (!frontier.Empty()) {
        current = frontier.GetState();      // lowest estimated_cost point
